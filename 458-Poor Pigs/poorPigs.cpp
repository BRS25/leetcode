...
Note:
    ...
    2) After a pig has instantly finished drinking buckets, there has to be a cool down time of m minutes. 
...
public:
    int poorPigs(int buckets, int minutesToDie, int minutesToTest) {
        ...
        if(minutesToDie<=minutesToTest){
            double exp = log10(buckets)/log10(base);
            double decPart, intpart;
            decPart = modf(exp, &intpart);
            if (decPart==0){
                return (int)exp;
            } else{
                exp++;
